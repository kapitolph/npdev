...
export async function cmdStatus(machine: Machine, opts: { json?: boolean } = {}): Promise<void> {
  const [sessions, repos] = await Promise.all([fetchSessions(machine), fetchRepos(machine)]);
  ...
  const activeUsers = [
    ...new Set(activeSessions.flatMap((s) => (s.attached_users || "").split(",").filter(Boolean))),
  ];
  ...
  // Sessions summary
  console.log(
    `Sessions: ${sessions.length} total (${activeSessions.length} active, ${idleSessions.length} idle, ${staleSessions.length} stale)`,
  );
  if (activeSessions.length > 0) {
    for (const s of activeSessions) {
      const users = (s.attached_users || "").split(",").filter(Boolean).join(",");
      console.log(
        `  ${s.name.padEnd(20)}  ${s.owner.padEnd(10)}  ${relativeTime(s.last_activity)}  [${users}]`,
      );
    }
  }
  console.log();
  ...
  for (const r of repos) {
    ...
    console.log(
      `  ${r.name.padEnd(24)}  ${r.branch.padEnd(12)}  ${count > 0 ? `${count} session(s)` : "-"}`,
    );
  }
}
